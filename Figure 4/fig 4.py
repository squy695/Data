...
ccu_cases = ['Condition 1 + 1×workload', 'Condition 1 + 2×workload']
...
for m_idx, method in enumerate(methods):
    ...
    for case in ccu_cases:
        try:
            df = pd.read_csv(f"{method}/{case}.csv", nrows=50)
...
if all(legend_handles):
    ax.legend(legend_handles, ccu_cases, loc='upper right', fontsize=24, frameon=True)
