...
# Data extraction for correlation analysis
X, Y = [], []
...
# Load quantified latency data
q_file = "quantified workload/avg latency HTTP 200.csv"
...
def analyze_spearman(X_res, Y_raw, max_lag=6):
    # Calculate Spearman correlation across negative lags (Y lagging X)
    corrs = []
...
def analyze_pearson(X_res, Y_raw, max_lag=6):
    # Calculate Pearson correlation across negative lags
    corrs = []
    lags = range(-max_lag, 0)
    for lag in lags:
        x_slice = X_res[:len(Y_raw) + lag]
...
# Perform time-series correlation analysis with resampling
corr_results = []
...
ax2.set_ylabel('Latency HTTP 200 (ms)', fontsize=32, fontweight='bold')
...
plt.tight_layout()
plt.savefig("fig_latency_HTTP_200_vs_workload.pdf", format='pdf', dpi=300, bbox_inches='tight')
plt.show()