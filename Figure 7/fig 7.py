...
# Load quantified workload data from single source
q_file = "quantified workload/quantified workload.csv"
...
def analyze_spearman(X_res, Y_raw, max_lag=6):
    # Calculate Spearman correlation across negative lags (Y lags X)
    corrs = []
...
# Plot Quantified Workload on secondary Y-axis
...
ax2.set_ylabel('Quantified workload (AOL = 1s)', fontsize=32, fontweight='bold')
...
plt.tight_layout()
plt.savefig("fig_7.pdf", format='pdf', dpi=300, bbox_inches='tight')
plt.show()