...
# Load and normalize actual workload
for path in input_dir:
    if os.path.exists(path):
        df = pd.read_csv(path)
        X.append(df['workload'] / 20)

# Load RPS data for quantified workload
q_file = "quantified workload/RPS HTTP 500.csv"
...
def analyze_spearman(X_res, Y_raw, max_lag=6):
    # Calculate Spearman correlation with time-lag shifting
    corrs = []
...
def analyze_pearson(X_res, Y_raw, max_lag=6):
    # Calculate Pearson correlation with time-lag shifting
    corrs = []
    lags = range(-max_lag, 0)
    for lag in lags:
        x_slice = X_res[:len(Y_raw) + lag]
...
# Execute resampled correlation analysis
corr_results = []
...
# Plot Quantified RPS on secondary Y-axis
...
ax2.set_ylabel('RPS HTTP 500', fontsize=32, fontweight='bold')
ax2.tick_params(axis='y', labelsize=28)

# Legend alignment and grid formatting
...
plt.tight_layout()
plt.savefig("fig_RPS_HTTP_500_vs_workload.pdf", format='pdf', dpi=300, bbox_inches='tight')
plt.show()