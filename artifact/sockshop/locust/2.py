...
class httpUser(HttpUser):
    ...
    @task
    def load(self):        
        self.client.get("/login", headers={"Authorization":"Basic %s" % auth_header, "Connection": "close"})
...
class DynamicStress(LoadTestShape):
    ...
    """
    wave_df = pd.read_csv('sockshop/locust/wiki/2.csv')
    ...
    def tick(self):
        ...
        if run_time < self.time_limit:
            user_count = self.wave_list[index] * 0.25 / 20
