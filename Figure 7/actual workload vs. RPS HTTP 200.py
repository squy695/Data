...
# Load quantified RPS data
q_file = "quantified workload/RPS HTTP 200.csv"
...
def analyze_spearman(X_res, Y_raw, max_lag=6):
    # Spearman correlation with negative lags (Y lags X)
    corrs = []
...
def analyze_pearson(X_res, Y_raw, max_lag=6):
    # Pearson correlation with negative lags (Y lags X)
    corrs = []
    lags = range(-max_lag, 0)
    for lag in lags:
        x_slice = X_res[:len(Y_raw) + lag]
...
# Plot Actual Workload on primary axis
for i in range(5):
...
# Plot Quantified RPS on secondary axis
...
ax2.set_ylabel('RPS HTTP 200', fontsize=32, fontweight='bold')
...
plt.tight_layout()
plt.savefig("fig_RPS_HTTP_200_vs_workload.pdf", format='pdf', dpi=300, bbox_inches='tight')
plt.show()